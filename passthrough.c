...
static int ptfs_chmod(const char *path, mode_t mode, struct fuse_file_info *fi)
{
    ptfs_impl_fullpath(path);
...
static int ptfs_chown(const char *path, uid_t uid, gid_t gid, struct fuse_file_info *fi)
{
    ptfs_impl_fullpath(path);
...
static int ptfs_read(const char *path, char *buf, size_t size, off_t off,
...
static int ptfs_write(const char *path, const char *buf, size_t size, off_t off,
    struct fuse_file_info *fi)
{
    int fd = fi_fd(fi);
...
static int ptfs_readdir(const char *path, void *buf, fuse_fill_dir_t filler, off_t off,
